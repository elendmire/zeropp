...
| src/zeropp/models/ar_emos.py | splits.py output |
| src/zeropp/models/ts_emos.py | splits.py output |
| src/zeropp/models/lead_time_continuous.py | splits.py output |
...
| src/zeropp/models/tsfm_citras.py | SSH `citras-fm` package install + verified covariate API |
...
| src/zeropp/eval/results.py | real model predictions + timing existing |
| src/zeropp/cli.py | the scripts it wraps |
| scripts/06_make_report.py | tables.py, figures.py |

Note: wall-clock timing (CLAUDE.md's mandatory metric) is not a separate
stub module — it's just `time.perf_counter()` calls at each script's call
site, and is scripts/03_run_baselines.py / 04_run_tsfm.py / 05_data_size_sweep.py's
responsibility once they're implemented for real.
